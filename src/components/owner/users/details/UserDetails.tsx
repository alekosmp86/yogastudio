...
export default function UserDetails({ id }: UserOverviewCardProps) {
  ...
  const moduleComponents = useUISlot(CoreUiSlots.OwnerUsersDetails);
      ...
      <div className='rounded-2xl bg-custom-50 p-5 mt-6 shadow-sm space-y-4'>
        ...
        <div className='grid grid-cols-2 gap-3'>
          ...
          <StatCard
            label={t("penalties")}
            value={`${
              user?.activityStats.penalties ?? 0
            }/${maxPenaltiesAllowed}`}
          />
        </div>
        ...
        {/** Entry point for modules' components */}
        {moduleComponents.length > 0 && (
          <>
            {moduleComponents.map((Component, i) => (
